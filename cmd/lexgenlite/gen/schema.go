...
func (s *Schema) AllTypes(prefix string, defMap map[string]*ExtDef) []outputType {
	...
	walk = func(name string, ts *TypeSchema, needsCbor bool) {
		...
		atproto_defs := []string{
			"LexiconSchema",
			"RepoStrongRef",
			"SyncSubscribeRepos_Commit",
			"SyncSubscribeRepos_Sync",
			"SyncSubscribeRepos_Identity",
			"SyncSubscribeRepos_Account",
			"SyncSubscribeRepos_Info",
			"SyncSubscribeRepos_RepoOp",
			"LabelDefs_SelfLabels",
			"LabelDefs_SelfLabel",
			"LabelDefs_Label",
			"LabelSubscribeLabels_Labels",
			"LabelSubscribeLabels_Info",
			"LabelDefs_LabelValueDefinition",
			"LabelDefs_LabelValueDefinitionStrings",
		}
		if slices.Contains(atproto_defs, name) {
			ts.needsCbor = true
		}
		...
		bsky_defs := []string{
			...
			"FeedLike",
			"RichtextFacet",
			"RichtextFacet_ByteSlice",
			"RichtextFacet_Link",
			"RichtextFacet_Mention",
			"RichtextFacet_Tag",
			...
			"FeedGenerator",
			"GraphListblock",
			"EmbedDefs_AspectRatio",
			"FeedThreadgate",
			"FeedThreadgate_ListRule",
			"FeedThreadgate_MentionRule",
			"FeedThreadgate_FollowerRule",
			"FeedThreadgate_FollowingRule",
			"GraphStarterpack_FeedItem",
			"GraphStarterpack",
			"LabelerService",
			"LabelerDefs_LabelerPolicies",
			"EmbedVideo",
			"EmbedVideo_Caption",
			"FeedPostgate",
			"FeedPostgate_DisableRule",
			"GraphVerification",
		}
		if slices.Contains(bsky_defs, name) {
			ts.needsCbor = true
		}

		if name == "EmbedImages_Image" ||
			name == "EmbedVideo_Caption" {
			ts.needsCbor = true
		}
