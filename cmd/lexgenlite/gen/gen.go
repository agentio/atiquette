// Package gen generates Go code for lexicons.
package gen
...
func GenCodeForSchema(pkg Package, reqcode bool, s *Schema, packages []Package,
	defmap map[string]*ExtDef) ([]outputType, error) {
	err := os.MkdirAll(pkg.Outdir, 0755)
	if err != nil {
		return nil, fmt.Errorf("%s: could not mkdir, %w", pkg.Outdir, err)
	}
	...
	pf("// Code generated by cmd/lexgenlite (see Makefile's lexgen); DO NOT EDIT.\n\n")
	...
	pf("\t\"fmt\"\n")
	pf("\t\"encoding/json\"\n")
	pf("\tcbg \"github.com/agentio/atiquette/pkg/typegen\"\n")
	pf("\t\"github.com/agentio/atiquette/pkg/xrpc\"\n")
	pf("\t\"github.com/agentio/atiquette/pkg/lexutil\"\n")
	...
	log.Printf("calling alltypes with prefix %s", pkg.Prefix)
...
func Run(schemas []*Schema, packages []Package, maxStringLength int) error {
	...
	for _, pkg := range packages {
		...
		for _, s := range schemas {
			...
			var err error
			if tps, err = GenCodeForSchema(pkg, true, s, packages, defmap); err != nil {
		...
		if false {
			b, _ := json.MarshalIndent(alltps, "", "  ")
			os.WriteFile("alltypes-"+pkg.Prefix+".json", b, 0644)
			log.Printf("writing %s %d", "alltypes-"+pkg.Prefix+".json", len(b))
		}
